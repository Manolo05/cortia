    ...
    <div style={{ background: '#F7F4EE', color: '#0B1D3A', fontFamily: "'DM Sans', system-ui, sans-serif", minHeight: '100vh' }}>
      ...
      <nav style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 50, backdropFilter: 'blur(20px)', background: 'rgba(247,244,238,0.85)', borderBottom: '1px solid rgba(11,29,58,0.05)' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: 68 }}>
          ...
          <button onClick={goApp} style={{ background: '#0B1D3A', color: '#fff', padding: '10px 24px', borderRadius: 8, fontSize: 14, fontWeight: 600, border: 'none', cursor: 'pointer' }}>{"Accéder à l'app"}</button>
        </div>
      </nav>
      ...
      <section style={{ paddingTop: 140, paddingBottom: 80, position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 24px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 64, alignItems: 'center' }}>
          <div>
            ...
            <p style={{ fontSize: 18, color: '#5A6B80', lineHeight: 1.6, marginBottom: 36, maxWidth: 480 }}>CortIA score vos dossiers, détecte les risques et recommande les meilleures banques.</p>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 16 }}>
              <button onClick={goApp} style={{ background: '#0B1D3A', color: '#fff', padding: '16px 32px', borderRadius: 10, fontSize: 15, fontWeight: 600, border: 'none', cursor: 'pointer' }}>Démarrer gratuitement →</button>
              <button onClick={() => scrollTo('how')} style={{ background: 'transparent', color: '#0B1D3A', padding: '16px 32px', borderRadius: 10, fontSize: 15, fontWeight: 600, border: '1.5px solid rgba(11,29,58,0.15)', cursor: 'pointer' }}>Voir la démo</button>
            </div>
          </div>
          <div style={{ position: 'relative' }}>
            <div className="lp-float" style={{ position: 'absolute', top: -10, right: -20, background: '#fff', padding: '10px 16px', borderRadius: 10, boxShadow: '0 8px 32px rgba(11,29,58,0.1)', fontSize: 13, fontWeight: 600, display: 'flex', alignItems: 'center', gap: 8, zIndex: 10 }}><span style={{ color: '#1D9E75' }}>✓</span> Score 87/100</div>
            <div style={{ background: '#fff', borderRadius: 20, padding: 28, boxShadow: '0 24px 80px rgba(11,29,58,0.08)', border: '1px solid rgba(11,29,58,0.04)' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}><span style={{ fontWeight: 600, fontSize: 14 }}>Dossier #2024-0847</span><span style={{ padding: '4px 12px', borderRadius: 9999, fontSize: 12, fontWeight: 600, background: 'rgba(29,158,117,0.1)', color: '#1D9E75' }}>Éligible</span></div>
              ...
              <div><p style={{ fontSize: 12, color: '#5A6B80', fontWeight: 500, marginBottom: 8 }}>{"Probabilité d'acceptation"}</p><div style={{ height: 8, background: 'rgba(11,29,58,0.06)', borderRadius: 9999, overflow: 'hidden' }}><div style={{ height: '100%', borderRadius: 9999, background: 'linear-gradient(to right, #1D9E75, #2CC98F)', transition: 'width 2s ease-out', width: `${barWidth}%` }} /></div></div>
              <div style={{ marginTop: 16 }}><p style={{ fontSize: 12, color: '#5A6B80', fontWeight: 500, marginBottom: 8 }}>Banques recommandées</p><div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}><span style={{ padding: '6px 14px', borderRadius: 8, fontSize: 12, fontWeight: 600, background: 'rgba(212,168,67,0.15)', border: '1px solid rgba(212,168,67,0.2)', color: '#D4A843' }}>Crédit Agricole ★</span><span style={{ padding: '6px 14px', borderRadius: 8, fontSize: 12, fontWeight: 600, background: '#F7F4EE', border: '1px solid rgba(11,29,58,0.06)' }}>BNP</span><span style={{ padding: '6px 14px', borderRadius: 8, fontSize: 12, fontWeight: 600, background: '#F7F4EE', border: '1px solid rgba(11,29,58,0.06)' }}>CIC</span></div></div>
            </div>
            <div className="lp-float2" style={{ position: 'absolute', bottom: 28, left: -28, background: '#fff', padding: '10px 16px', borderRadius: 10, boxShadow: '0 8px 32px rgba(11,29,58,0.1)', fontSize: 13, fontWeight: 600, display: 'flex', alignItems: 'center', gap: 8, zIndex: 10 }}><span style={{ color: '#D4A843' }}>⚡</span> 2 min</div>
          </div>
        </div>
      </section>
      ...
      <section style={{ padding: '64px 0', borderTop: '1px solid rgba(11,29,58,0.05)' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 24px', display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 40, textAlign: 'center' }}>
          {[['3x','Plus rapide'],['87%','Prédiction'],['2 min','Par dossier'],['35k+','Courtiers IOBSP']].map(([v,l])=>(<div key={v as string}><div className="lp-serif" style={{ fontSize: 36, color: '#0B1D3A' }}>{v}</div><div style={{ fontSize: 14, color: '#5A6B80', marginTop: 4 }}>{l}</div></div>))}
        </div>
      </section>
      ...
      <section id="features" style={{ padding: '96px 0' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 24px' }}>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: 'rgba(212,168,67,0.15)', color: '#D4A843', padding: '6px 16px', borderRadius: 9999, fontSize: 13, fontWeight: 600, marginBottom: 16, border: '1px solid rgba(212,168,67,0.15)' }}>Fonctionnalités</div>
          <h2 className="lp-serif" style={{ fontSize: 40, lineHeight: 1.15, marginBottom: 16, maxWidth: 600 }}>{"Tout ce dont un courtier a besoin, augmenté par l'IA"}</h2>
          <p style={{ fontSize: 17, color: '#5A6B80', lineHeight: 1.6, maxWidth: 520, marginBottom: 64 }}>CortIA centralise et automatise votre métier.</p>
      ...
      <section id="how" style={{ padding: '96px 0', background: '#0B1D3A', color: '#fff', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 24px' }}>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: 'rgba(212,168,67,0.1)', color: '#D4A843', padding: '6px 16px', borderRadius: 9999, fontSize: 13, fontWeight: 600, marginBottom: 16, border: '1px solid rgba(212,168,67,0.15)' }}>Comment ça marche</div>
          <h2 className="lp-serif" style={{ fontSize: 40, lineHeight: 1.15, marginBottom: 16, maxWidth: 600 }}>3 étapes simples</h2>
      ...
      <section style={{ padding: '96px 0', background: '#F7F4EE' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 24px' }}>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: 'rgba(212,168,67,0.15)', color: '#D4A843', padding: '6px 16px', borderRadius: 9999, fontSize: 13, fontWeight: 600, marginBottom: 16, border: '1px solid rgba(212,168,67,0.15)' }}>Témoignages</div>
          <h2 className="lp-serif" style={{ fontSize: 40, lineHeight: 1.15, marginBottom: 16 }}>Ils ont adopté CortIA</h2>
      ...
      <section style={{ padding: '96px 0' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 24px' }}>
          <div style={{ background: '#0B1D3A', borderRadius: 24, padding: '80px 64px', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
            ...
            <h2 className="lp-serif" style={{ fontSize: 40, color: '#fff', marginBottom: 16, position: 'relative' }}>Prêt à transformer votre courtage ?</h2>
            ...
            <button onClick={goApp} style={{ background: '#D4A843', color: '#0B1D3A', padding: '18px 40px', borderRadius: 10, fontSize: 16, fontWeight: 600, border: 'none', cursor: 'pointer', position: 'relative' }}>Démarrer mon essai gratuit →</button>
          </div>
        </div>
      </section>

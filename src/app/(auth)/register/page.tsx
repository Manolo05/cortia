...
export default function RegisterPage() {
  ...
  async function handleRegister(e: React.FormEvent) {
    ...
    try {
      // Step 1: Create user via server API (bypasses DB triggers)
  ...
  if (success) {
    return (
      <div className="animate-fade-in text-center">
        ...
        <p className="text-slate-500 mb-6">Connectez-vous pour accéder à votre espace CortIA</p>
  ...
  return (
    <div className="animate-fade-in">
      <div className="mb-8">
        <div className="w-12 h-12 rounded-2xl flex items-center justify-center mb-6 lg:hidden"
      ...
      <form onSubmit={handleRegister} className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <input type="text" value={formData.nomComplet}
              ...
              className="cortia-input w-full" placeholder="Jean Dupont" required disabled={loading} />
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-2">Nom du cabinet</label>
            <input type="text" value={formData.nomCabinet}
              onChange={e => setFormData(prev => ({ ...prev, nomCabinet: e.target.value }))}
              className="cortia-input w-full" placeholder="Mon Cabinet" required disabled={loading} />
          </div>
        </div>
        ...
        <div>
          ...
          <input type="email" value={formData.email}
            ...
            className="cortia-input w-full" placeholder="votre@email.com" required disabled={loading} />
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <input type="password" value={formData.password}
              ...
              className="cortia-input w-full" placeholder="Min. 6 caractères" required minLength={6} disabled={loading} />
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-2">Confirmer</label>
            <input type="password" value={formData.confirmPassword}
              onChange={e => setFormData(prev => ({ ...prev, confirmPassword: e.target.value }))}
              className="cortia-input w-full" placeholder="Répéter" required minLength={6} disabled={loading} />
          </div>
        </div>
      ...
      <p className="mt-6 text-center text-sm text-slate-500">
        Déjà un compte ?{' '}
        <Link href="/login" className="font-semibold text-blue-600 hover:text-blue-700">Se connecter</Link>
      </p>
    </div>
  )
}

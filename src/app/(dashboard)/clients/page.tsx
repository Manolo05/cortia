  ...
  return (
        <div className="page-container">
              <div className="page-header">
                      <div>
                                <h1 className="page-title">Clients</h1>h1>
                                <p className="page-subtitle">{clients.length} client{clients.length > 1 ? 's' : ''} au total</p>p>
                      </div>div>
                      <Link href="/dossiers/nouveau" className="btn-primary">+ Nouveau dossier</Link>Link>
              </div>div>
              ...
              <div style={{ display: 'flex', gap: '0.75rem', marginBottom: '1.5rem', alignItems: 'center' }}>
                      <div style={{ position: 'relative', flex: '1', maxWidth: '400px' }}>
                                <input
                                            ...
                                            />
                                <span style={{ position: 'absolute', left: '0.75rem', top: '50%', transform: 'translateY(-50%)', color: 'var(--text-muted)', fontSize: '0.875rem' }}>◯</span>span>
                      </div>div>
                {search && (
                                    ...
                                    <tr key={c.id} onClick={() => window.location.href = `/dossiers/${c.dossier_id}`} style={{ cursor: 'pointer' }}>
                                                      <td>
                                                                          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                                                                                                ...
                                                                                                <div>
                                                                                                                        <div style={{ fontWeight: 600, color: 'var(--text-primary)' }}>{c.nom_complet}</div>div>
                                                                                                  {c.role === 'co_emprunteur' && (
                                                                <div style={{ fontSize: '0.7rem', color: 'var(--text-muted)' }}>Co-emprunteur</div>div>
                                                                                                                        )}
                                                                                                  </div>div>
                                                                          </div>div>
                                                      </td>td>
                                                      <td>
                                                                          <div style={{ fontSize: '0.8rem' }}>
                                                                            {c.email && <div style={{ color: 'var(--text-secondary)' }}>{c.email}</div>div>}
                                                                            {c.telephone && <div style={{ color: 'var(--text-muted)', marginTop: '0.1rem' }}>{c.telephone}</div>div>}
                                                                            {!c.email && !c.telephone && <span style={{ color: 'var(--text-muted)' }}>—</span>span>}
                                                                          </div>div>
                                                      </td>td>
                                                      <td>
                                                      ...
                                                      <td style={{ fontWeight: 500 }}>{formatCurrency(c.salaire_net)}/mois</td>td>
                                                      <td>
